...
CLASS lcl_appl IMPLEMENTATION.
  ...
  METHOD open_int_table.
    ...
    ENDIF.
    APPEND INITIAL LINE TO lcl_appl=>mt_obj ASSIGNING FIELD-SYMBOL(<obj>).
    <obj>-alv_viewer = NEW #(  i_additional_name = iv_name ir_tab = r_tab i_tname = i_tname ).
    <obj>-alv_viewer->mo_sel->raise_selection_done( ).
...
CLASS lcl_main_tree DEFINITION FINAL INHERITING FROM lcl_popup.
  PUBLIC SECTION.
    TYPES: BEGIN OF ts_table,
             tabname TYPE string,
             ref     TYPE REF TO data,
...
CLASS lcl_main_tree IMPLEMENTATION.
  METHOD constructor.
    ...
    lo_columns->get_column( 'TABNAME' )->set_short_text( 'TABNAME' ).
    ...
    SET HANDLER hndl_double_click FOR lo_event.
*                hndl_user_command FOR lo_event.
  ...
  METHOD add_persons_tables.
    ...
    LOOP AT lt_t777d INTO DATA(ls_t777d).
      ...
      CHECK ls_t777d-dbtab CP 'PA*'. " OR ls_t777d-dbtab = 'HRP1001'.
  ...
  METHOD add_table.
    ...
    CREATE DATA lr_tbldata TYPE TABLE OF (iv_table).
    ASSIGN lr_tbldata->* TO <fs_tbl>.
    CHECK <fs_tbl> IS ASSIGNED.
